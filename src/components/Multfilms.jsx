  ...
  if (showFavorites) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-purple-900 via-pink-900 to-gray-900 text-white p-6">
        ...
        <h1 className="text-3xl font-bold mb-6 text-center">❤️ Favourite Cartoons</h1>
        {favorites.length === 0 ? (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-900 via-pink-800 to-gray-900 text-white p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <h2 className="text-3xl font-bold text-yellow-400 text-center flex-1">🎞 Cartoons</h2>
        <button
      ...
      <div className="flex justify-center mb-8 relative w-full max-w-md mx-auto">
        <input
          ...
          className="w-full px-5 py-3 rounded-full bg-gray-800 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-400 transition"
        />
        {searchInput && (
          <button
            onClick={() => {
      ...
      {movies.length === 0 ? (
        <p className="text-center text-gray-300 mt-10 animate-pulse">⏳ Loading cartoons...</p>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {movies.map((movie) => {
              ...
              return (
                <div
                  key={movie.id}
                  className="bg-gray-800 rounded-2xl overflow-hidden shadow-lg cursor-pointer hover:scale-105 hover:shadow-yellow-400/30 transition-transform duration-300 relative"
                  onClick={() => setSelectedMovie(movie)}
                >
                  <img
                    src={
                  ...
                  <div className="p-3 text-center">
                    ...
                    <p className="text-yellow-400 mt-1">⭐ {movie.vote_average?.toFixed(1) || "N/A"}</p>
                  </div>
                </div>
              );
            })}
          ...
          {loading && <p className="text-center text-yellow-400 mt-4 animate-pulse">Loading...</p>}
        </>
      )}
    </div>
  );
